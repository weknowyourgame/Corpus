...
const META_RE = /\b(what\s+can\s+you\s+do|what\s+do\s+you\s+do|how\s+do\s+you\s+work|what\s+are\s+you|who\s+are\s+you|are\s+you\s+(claude|gpt|an?\s+ai|a\s+bot)|what('s|\s+is)\s+(your\s+(name|purpose)|corpus)|what\s+(model|llm|version)\s+(are|is)\s+you)\b/i;
