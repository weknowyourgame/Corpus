...
/** Extract and validate X-Stud-Token header.
...
const requireToken = (req, res) => {
  const token = (req.header("X-Stud-Token") || "").trim();
  if (!token) {
...
app.post("/auth/studio-token/generate", (req, res) => {
  ...
  if (oldToken && studioTokens.has(oldToken)) {
    studioTokens.delete(oldToken);
  }
