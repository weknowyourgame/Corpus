import { CorpusLogo } from "./CorpusLogo";
import { CorpusStatusPill } from "./CorpusStatusPill";
...
function UserBadge() {
  ...
  return (
    ...
    >
      ...
      <span className="max-w-[96px] truncate" style={{ color: "var(--corpus-muted)" }}>{label}</span>
    </button>
  );
}

export function CorpusAppHeader({
  status,
  transport,
  trailing,
  compact = false,
  ...
  return (
    <header className="corpus-app-header">
      <div className="corpus-brand-lockup">
        <CorpusLogo large={compact} />
        <span className="corpus-brand-subtitle">Roblox Studio Agent</span>
      </div>
      <div className="corpus-app-header-actions">
        {status !== undefined && (
          <CorpusStatusPill
