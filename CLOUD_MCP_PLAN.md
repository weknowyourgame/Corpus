...
Stud should run as a cloud MCP server. Users install the Roblox Studio plugin once, paste a Stud token once, and then any connected client can modify their local Studio through Stud Cloud.

No local Node bridge. No session code flow. No official local StudioMCP dependency. The Stud plugin is the only local executor.
...
- `/stud/sessions/:id/*`
- `/stud/request`, `/stud/poll`, `/stud/respond`
- `public/studio-plugin/stud-bridge.server.lua`
- `server/agent/mcp-stdio.ts`
- `OfficialMcpTransport`
- `CompositeStudioTransport`
- `STUD_STUDIO_TRANSPORT`
- `STUD_STUDIO_MCP_BINARY`
...
```txt
You are implementing Cloud MCP for Stud.

Goal:
Make Stud a cloud-hosted MCP server for Roblox Studio. Remove active reliance on the local Node bridge, session codes, official local StudioMCP, and path-based plugin protocol. The only local component should be the Stud Roblox Studio plugin, which authenticates with a Studio token and long-polls the cloud for tool calls.
...
3. Remove `OfficialMcpTransport`, `CompositeStudioTransport`, `mcp-stdio.ts`, and `STUD_STUDIO_TRANSPORT` from the active server flow.
4. Implement `PluginRelayTransport` as the only Studio transport.
5. Replace `/stud/token/poll` and `/stud/token/respond` with cloud endpoints:
   - `GET /studio/poll`
   - `POST /studio/respond`
6. Poll must authenticate with `Authorization: Bearer <studio_token>` or `X-Stud-Token`.
...
14. Remove or ignore legacy session-code routes and `public/studio-plugin/stud-bridge.server.lua`.
15. Add tests for:
   - token auth
